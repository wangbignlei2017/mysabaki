...
class InfoDrawer extends Component {
    constructor() {
        ...
        this.handleSubmitButtonClick = evt => {
            evt.preventDefault()
            if(this.state.blackName ===null || this.state.blackName ==="")
            {   
                dialog.showMessageBox(`请输入黑方名字`)
                return
            }
            if(this.state.whiteName ===null || this.state.whiteName ==="")
            {   
                dialog.showMessageBox(`请输入白方名字`)
                return
            }
